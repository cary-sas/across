...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
		...
		~/.acme.sh/acme.sh   --register-account -m admin@$DOMAIN
		~/.acme.sh/acme.sh --set-default-ca  --server zerossl
            ~/.acme.sh/acme.sh  --force --issue -d $DOMAIN --pre-hook "nginx -s stop || { echo -n ''; }" --post-hook "nginx -c /www/server/nginx/conf/nginx.conf || { echo -n ''; }"  --standalone
        fi
        ...
        CERT_FILE="/etc/trojan-go/${DOMAIN}.pem"
        KEY_FILE="/etc/trojan-go/${DOMAIN}.key"
        ~/.acme.sh/acme.sh  --force --install-cert -d $DOMAIN \
            --key-file       $KEY_FILE  \
